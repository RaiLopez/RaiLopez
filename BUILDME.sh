...
if [ -s "$CATALOG_DATA" ]; then
	...
	{ echo "$LINE"; echo "$LINES"; } | while IFS="|" read -r id name ver bld dsc tar url; do
		...
		# --- 🖼️ TRIPLE ICON/FALLBACK LOGIC (Hybrid structure)
		IMG_MAIN="${URL_RAW_MONO}/docs/assets/icon_fallback.png"
		IMG_DARK="${URL_RAW_MONO}/docs/assets/icon_fallback_dark.png"
		IMG_LIGHT="${URL_RAW_MONO}/docs/assets/icon_fallback_light.png"
		ASSETS_PATH="./docs/${id}/assets"

		if [ -f "$ASSETS_PATH/icon.png" ]; then # Comprobamos existencia de iconos específicos del pack
			IMG_MAIN="${URL_RAW_MONO}/docs/${id}/assets/icon.png" # El icono base siempre será el original de Moho
			if [ -f "$ASSETS_PATH/icon_dark.png" ]; then # Buscamos versión optimizada para DARK
				IMG_DARK="${URL_RAW_MONO}/docs/${id}/assets/icon_dark.png"
			else
				IMG_DARK="$IMG_MAIN" # Fallback al original si no hay dark
			fi
			if [ -f "$ASSETS_PATH/icon_light.png" ]; then # Buscamos versión optimizada para LIGHT (la que necesita GitHub)
				IMG_LIGHT="${URL_RAW_MONO}/docs/${id}/assets/icon_light.png"
			else
				IMG_LIGHT="$IMG_MAIN" # Fallback al original si no hay light
			fi
		fi
		...
		echo "| [$PICTURE_TAG](${PACK_LNK}) | $DISPLAY_NAME | $DISPLAY_DESC | [ &nbsp;⏬&nbsp; ]($url) |" >> "$TEMP_TABLE"
	done
