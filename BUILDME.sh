...
if [ -s "$CATALOG_DATA" ]; then
    ...
    { 
        ...
        # --- 🔗 GESTIÓN DE LINKS (Punto 2)
        # Para el Core usamos un link relativo al directorio actual o al top del README
        if [[ "$id" == "$CORE" ]]; then
            PACK_LNK="#top" 
        else
        ...
        # --- 🖼️ GESTIÓN DE ICONOS (Punto 1 y 3)
        # Definimos la ruta local donde el Builder DEBERÍA encontrar el icono
        # Para el Core está en ScriptResources/ls/... para los packs en ../id/ScriptResources/id/...
        if [[ "$id" == "$CORE" ]]; then
            LOCAL_RSC_PATH="./ScriptResources/${CORE}"
            REMOTE_RSC_URL="${URL_RAW_CORE}"
        else
            LOCAL_RSC_PATH="../${id}/ScriptResources/${id}"
            REMOTE_RSC_URL="${URL_RAW}/${id}/main/ScriptResources/${id}"
        fi

        # Lógica de cascada: webp -> png -> fallback
        if [ -f "${LOCAL_RSC_PATH}/${id}@2x.webp" ]; then
            ICON_URL="${REMOTE_RSC_URL}/${id}@2x.webp"
        elif [ -f "${LOCAL_RSC_PATH}/${id}@2x.png" ]; then
            ICON_URL="${REMOTE_RSC_URL}/${id}@2x.png"
        else
            ICON_URL="${URL_RAW_CORE}/ls_fallback@2x.webp"
        fi

        # --- ✨ CUSTOMIZATION
        if [[ "$id" == "$CORE" ]]; then
            DISPLAY_NAME="[***LS&nbsp;<sup>Core</sup>***](${PACK_LNK} 'Go to top...') "
            DISPLAY_DESC="***<sup>Essential shared/common resources, utils, and core modules required by the [Lost Scripts™](https://lost-scripts.github.io/) project for MOHO<sup> Pro</sup> Animation Software.&nbsp;</sup>***"
        else
        ...
        # Output final sin 'onerror' (Punto 1)
        echo "| [<img src='${ICON_URL}' width='48'>](${PACK_LNK}) | $DISPLAY_NAME | $DISPLAY_DESC | [ &nbsp;⏬&nbsp; ]($url 'Download: ${id}.zip') |" >> "$TEMP_TABLE"
    done
