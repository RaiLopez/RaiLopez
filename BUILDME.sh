...
if [ -s "$CATALOG_DATA" ]; then
	LINE=$(grep "^${CORE}|" "$CATALOG_DATA") || true
	LINES=$(grep -v "^${CORE}|" "$CATALOG_DATA" | sort -t'|' -k2) || true
	
	{ echo "$LINE"; echo "$LINES"; } | while IFS="|" read -r id name ver bld dsc tar url; do
		[[ -z "$id" ]] && continue
		PACK_LNK="${URL_BASE}/${id}/"

		# --- 🔍 LÓGICA DE ICONO VS. FALLBACK (Decidida por el Builder)
		# 1. Definimos las rutas del Fallback Global (Monorepo)
		IMG_MAIN="${URL_RAW_MONO}/docs/assets/icon_fallback.png"
		IMG_DARK="${URL_RAW_MONO}/docs/assets/icon_fallback_dark.png"

		# 2. Comprobamos si el pack actual tiene su propia carpeta de assets e iconos
		# (Asumiendo que el script corre desde la raíz del Monorepo)
		if [ -f "./docs/${id}/assets/icon.png" ]; then
			IMG_MAIN="${URL_RAW_MONO}/docs/${id}/assets/icon.png"
			# Si tiene el claro, miramos si también tiene el oscuro específico
			if [ -f "./docs/${id}/assets/icon_dark.png" ]; then
				IMG_DARK="${URL_RAW_MONO}/docs/${id}/assets/icon_dark.png"
			else
				# Si no tiene oscuro, podemos usar el claro o el fallback_dark
				IMG_DARK="${URL_RAW_MONO}/docs/${id}/assets/icon.png" 
			fi
		fi

		# --- 🖼️ GENERACIÓN DEL TAG PICTURE (Limpio y robusto)
		PICTURE_TAG="<picture><source media='(prefers-color-scheme: dark)' srcset='${IMG_DARK}'><img src='${IMG_MAIN}' width='48'></picture>"

		# --- ✨ PERSONALIZACIÓN DE FILAS (Core vs Otros)
		if [[ "$id" == "$CORE" ]]; then
			DISPLAY_NAME="[***LS&nbsp;<sup>Core</sup>***](${PACK_LNK} 'Go to \"$CORE\" repo...') "
			DISPLAY_DESC="***<sup>Essential shared resources, utilities, and core modules required for the [Lost Scripts™](https://lost-scripts.github.io/) project to work with [MOHO](https://moho.lostmarble.com/)<sup>&nbsp;Pro</sup>.</sup>***"
		else
