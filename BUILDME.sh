...
if [ -s "$CATALOG_DATA" ]; then
    LINE=$(grep "^${CORE}|" "$CATALOG_DATA"); LINES=$(grep -v "^${CORE}|" "$CATALOG_DATA" | sort -t'|' -k2)
    { echo "$LINE"; echo "$LINES"; } | while IFS="|" read -r id name ver bld dsc tar url; do
        ...
        ICON_URL=""
        # 1. Search in main folders (Tool/Menu) then Resources
        for loc in "Tool" "Menu" "ScriptResources/${id}"; do
        ...
        if [[ "$id" == "$CORE" ]]; then
            DISPLAY_NAME="[***LS&nbsp;<sup>Core</sup>***](${PACK_LNK}) "
            DISPLAY_DESC="***<sup>Essential shared modules required by [Lost Scripts™](https://lost-scripts.github.io/) for MOHO<sup> Pro</sup>.&nbsp;</sup>***"
        else
            DISPLAY_NAME="[<sup>**$name**</sup>](${PACK_LNK})<br><sub><sup title='Build: $bld'>v$ver</sup></sub>"
            DISPLAY_DESC="<sup>$dsc</sup><br><sub><sup>ⓘ For Moho $tar</sup></sub>"
        fi

        echo "| [<img src='${ICON_URL}' width='48'>](${PACK_LNK}) | $DISPLAY_NAME | $DISPLAY_DESC | [ &nbsp;⏬&nbsp; ]($url) |" >> "$TEMP_TABLE"
    done
