...
| :--: | -- | -- | :--: |
| [<img src="${URL_RAW_CORE}/${CORE}@2x.png" width="48">](./${CORE}/ 'Go to \"$CORE\" (Core) repo...') | [***LS&nbsp;<sup>Core</sup>***](./${CORE}/ 'Go to \"$CORE\" (Core) repo...') | ***<sup>Essential shared resources, utils, and core modules required by [Lost Scripts™](https://lost-scripts.github.io/ "Go to Lost Scripts™ homepage...") project for Moho® Pro.</sup>*** | [ &nbsp;⏬&nbsp; ](${URL_BASE}/${CORE}/archive/refs/heads/main.zip 'Download: ${CORE}.zip') |
EOF
...
if [ -s "$CATALOG_DATA" ]; then
	while IFS="|" read -r id name ver bld dsc tar url; do
		...
		echo "| [<img src='${PACK_RAW}/${id}@2x.png' width='48' onerror=\"this.src='${FALLBACK}'\">](${PACK_LNK} 'Go to \"$id\" script repo...') | [<sup>**$name**</sup>](${PACK_LNK} 'Go to \"$id\" script repo...')<br><sub><sup title='Build: $bld'>v$ver</sup></sub> | <sup>$dsc</sup><br><sub><sup>🛈 For Moho $tar</sup></sub> | [ &nbsp;⏬&nbsp; ]($url 'Download: ${id}.zip') |" >> "$TEMP_TABLE"
	done < "$CATALOG_DATA"
